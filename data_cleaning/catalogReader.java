...
public class catalogReader
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        while ((line = bufferedReader.readLine()) != null)
        {
            ...
            if (i % 22 == 0 && !line.contains("'") && !line.contains("1.")
                    && !line.contains("2.") && !line.contains("3.") && !line.
                    contains("4.") && !line.contains("5.") && !line.contains(
                            "6.") && !line.contains("7.") && !line.
                    contains("8.") && !line.contains("9."))
            {
                j++;
                        ...
                        == ',' && line.charAt(oldComma + 1) == '"')
                {
                    ...
                    oldComma = end + 1;
                } else if (line.length() >= oldComma + 2 && line.
                        charAt(oldComma) == ',')
                {
                        ...
                        == ',')
                {
                    ...
                    if (isbn.length() > 0 && (isbn.charAt(isbn.length() - 1)
                            == '.' || isbn.charAt(isbn.length() - 1) == 'X'
                            || isbn.charAt(isbn.length() - 1) == '-' || isbn.
                            charAt(isbn.length() - 1) == '*' || !Character.
                            isDigit(isbn.charAt(isbn.length() - 1))))
                    {

                        isbn = isbn.substring(0, isbn.length() - 1);

                    }
                    if (isbn.length() > 0 && (isbn.contains("download") || isbn.
                            contains("sound")) || isbn.contains("none") || isbn.
                            contains("Ne"))
                    {
                        isbn = "null";
                    }
                ...
                if (isbn.length() == 0 || "".equals(isbn) || isbn == null
                        || isbn.contains("NONE") || (isbn.length() > 2
                        && !Character.isDigit(isbn.charAt(1))))
                {
                    isbn = "null";
                }
                if (title.length() == 0)
                ...
                if (!"null".equals(title) && !"null".equals(author) && !"null".
                        equals(edition) && isbn!=null)
                {
            ...
            String edition, String isbn)
    {
        ...
        while (i < numberOfBranchOccurences)
        {
            ...
            query
                    ...
                    + clean(title) + "," + clean(author) + "," + clean(edition)
                    + "," + libraryBranchID[i] + "," + clean(type) + "," + isbn
                    + "," + copies[i] + ");";
